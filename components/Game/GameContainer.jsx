...
export default function GameContainer({ intro, children }) {
  const { user } = UserAuth();
  ...
  return (
    <m.div
      className={`w-full sm:w-[640px] mx-auto h-full flex flex-col items-center  ${
        intro === "false" ? "gap-10 pt-[6rem]" : "gap-3"
      }`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
